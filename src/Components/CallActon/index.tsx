
export const CallAction = () => {
  return (
    <section className="w-full h-auto bg-gray-100 py-20 px-4 flex justify-center items-center">
      <div className="w-full h-96 p-6 container bg-greenTheme flex flex-col items-center justify-center gap-2 rounded-xl">
        <h3 className="text-sm font-semibold text-white text-center">
          Proteja Seus Direitos com Confiança e Excelência Jurídica
        </h3>
        <h1 className="text-xl md:text-4xl text-center text-white font-bold">
          Confie seu caso a quem transforma desafios em vitórias. Agende sua consulta agora e garanta a defesa que você merece!
        </h1>
        <button className="w-36 text-base bg-white px-4 py-2 mt-2 rounded text-greenTheme font-bold">Saiba Mais!</button>
      </div>
    </section>
  )
}

